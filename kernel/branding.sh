...
#    (gets canonicalized by the two-pass config step in kleaf.sh)
...
#    patch it directly so Kleaf picks up our values during kernel compile
...
# 4. Kleaf action_env flags (consumed by KLEAF_ARGS in build/kleaf.sh)
BRANDING_KLEAF_ARGS=(
